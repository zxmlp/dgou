...
<form class="navbar-form navbar-right" action="goods/goods_queryGoods" method="post">
    ...
    <table class="table">
      ...
      <tbody>
        ...
        <c:forEach var="moms" items="${momsList}" varStatus="status">
	          <tr>
	            <td><c:out value="${status.index+1}"></c:out></td>
	            <td><a href="moms/moms_showDetail?moms.momsid=${moms.momsid}"><c:out value="${moms.momsname}"></c:out></a></td>
	            <td><c:out value="${moms.mprice}"></c:out></td>
	            <td><a href="moms/moms_showEdit?moms.momsid=${moms.momsid}">编辑</a></td>
	            <td><a href="moms/moms_deleteMoms?moms.momsid=${moms.momsid}">删除</a></td>
	            <td><a href="order/order_addOrder?moms.momsid=${moms.momsid}&user.name=${user.name}">立即抢购</a></td>
	          </tr>
	        </c:forEach>
	      </table>
	    </s:form>    
